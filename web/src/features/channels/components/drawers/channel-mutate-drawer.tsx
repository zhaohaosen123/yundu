...
export function ChannelMutateDrawer({
  ...
  const [poolImportUpstreamModels, setPoolImportUpstreamModels] = useState<
    string[]
  >([])
  const [poolImportEnabled, setPoolImportEnabled] = useState(false)
  ...
  useEffect(() => {
    if (
      ...
      setPoolImportUpstreamModels((current) =>
        current.length > 0 ? current : discovery.models
      )
      form.setValue('pool_import_upstream_models', discovery.models, {
        shouldDirty: true,
      })
    }
  }, [discovery.status, discovery.models, form, isEditing, poolImportEnabled])
  ...
  const onSubmit = useCallback(
    async (data: ChannelFormValues) => {
      ...
      if (
        ...
        discovery.status === 'success' &&
        discovery.models.length > 0 &&
        poolImportUpstreamModels.length === 0
      ) {
        toast.error(t('Select at least one upstream model'))
        return
      }
    ...
    [
      ...
      poolImportUpstreamModels.length,
      poolImportEnabled,
    ]
  )
  ...
  >(
    (v, details) => {
      ...
      if (!v) {
        form.reset(CHANNEL_FORM_DEFAULT_VALUES)
        setPoolImportUpstreamModels([])
        setPoolImportEnabled(false)
        setClipboardConnectionInfo(null)
      }
    },
                ...
                >
                  <div className='flex items-start justify-between gap-3'>
                    <div className='min-w-0 space-y-1'>
                      <FormLabel required>
                        {t(
                          !isEditing && poolImportUpstreamModels.length > 0
                            ? 'Project models'
                            : 'Models'
                        )}
                      </FormLabel>
                      <FormDescription>
                        {t(
                          !isEditing && poolImportUpstreamModels.length > 0
                            ? 'Each selected project model is mapped to every selected upstream model.'
                  ...
                  {!isEditing && (
                    <div className='flex items-center justify-between rounded-md border px-3 py-2'>
                      <div>
                        <FormLabel>{t('Import as resource pool')}</FormLabel>
                        <FormDescription>
                          {t(
                            'Create one resource for each project and upstream model combination.'
                          )}
                        </FormDescription>
                      </div>
                      <Switch
                        checked={poolImportEnabled}
                        onCheckedChange={(checked) => {
                          setPoolImportEnabled(checked)
                          if (!checked) {
                            setPoolImportUpstreamModels([])
                            form.setValue('pool_import_upstream_models', [])
                          }
                        }}
                      />
                    </div>
                  ...
                  <FormControl>
                    <MultiSelect
                      ...
                      placeholder={t(
                        !isEditing && poolImportUpstreamModels.length > 0
                          ? 'Select project models or add custom ones'
                  ...
                  discovery.models.length > 0 && (
                    <div className='border-border/60 rounded-lg border p-3'>
                      ...
                      <UpstreamModelSelection
                        ...
                        selected={
                          isEditing || !poolImportEnabled
                            ? currentModelsArray
                            : poolImportUpstreamModels
                        }
                        existingModels={
                          isEditing ? initialModelsRef.current : []
                        }
                        onChange={(models) => {
                          if (isEditing || !poolImportEnabled) {
                            handleModelsChange(models)
                          } else {
                            setPoolImportUpstreamModels(models)
                            form.setValue(
                              'pool_import_upstream_models',
                              models,
                              { shouldDirty: true }
                            )
                          }
