...
export const channelFormSchema = z
  .object({
    ...
    pool_import_upstream_models: z.array(z.string()).optional(),
...
export function transformFormDataToCreatePayload(formData: ChannelFormValues): {
  mode: 'single' | 'batch' | 'multi_to_single' | 'model_pool'
  multi_key_mode?: 'random' | 'polling'
  batch_add_set_key_prefix_2_name?: boolean
  channel: Partial<Channel>
  pool_import?: { upstream_models: string[]; project_models: string[] }
} {
  const poolModels = formData.pool_import_upstream_models ?? []
  const mode =
    poolModels.length > 0 ? 'model_pool' : formData.multi_key_mode || 'single'
