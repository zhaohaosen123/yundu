...
function getEpayMethods(payMethods: PaymentMethod[] = []): PaymentMethod[] {
  return payMethods.filter((m) => m?.type === 'alipay' || m?.type === 'wxpay')
}
...
export function SubscriptionPlansCard({
  ...
  const enableStripe = false
  const enableCreem = false
  const enableWaffoPancake = false
                  ...
                  <CardContent className='flex h-full flex-col p-3.5 sm:p-4'>
                    ...
                    <div className='py-2'>
                      <span className='text-primary text-2xl font-bold'>
                        {formatLocalCurrencyAmount(Number(price), {
                          abbreviate: false,
                          digitsLarge: 2,
                          digitsSmall: 2,
                        })}
