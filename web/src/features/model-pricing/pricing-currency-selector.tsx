...
export function PricingCurrencySelector(props: {
  ...
  const value = available && preference === 'site' ? 'site' : 'site'
  const items: { value: 'site'; label: string }[] = []
  ...
  return (
    <Field className='mb-4 gap-2'>
      <div className='flex items-center gap-1'>
        ...
        >
          <p className='text-sm'>{t('All prices and payments use RMB.')}</p>
      ...
      {props.siteCurrency && !available && (
        <p id={`${id}-error`} className='text-muted-foreground text-xs'>
          {t('The site currency is unavailable.')}
        </p>
      )}
    </Field>
  )
