...
func GetTopUpInfo(c *gin.Context) {
	...
	// 获取支付方式
	payMethods := operation_setting.GetSupportedPayMethods()
...
func validateTopUpQuotaDecimal(amount decimal.Decimal) (int, error) {
	quota, err := getTopUpQuota(amount)
	if err == nil && quota > 0 {
		return quota, nil
	}
	maxAmount := getMaxTopUpAmount()
	if maxAmount > 0 && amount.GreaterThan(decimal.NewFromInt(maxAmount)) {
...
func validateTopUpQuota(amount int64) (int, error) {
	return validateTopUpQuotaDecimal(decimal.NewFromInt(amount))
}
...
func rejectInvalidTopUpQuotaDecimal(c *gin.Context, userId int, amount decimal.Decimal) bool {
	creditedQuota, err := validateTopUpQuotaDecimal(amount)
	if err == nil {
		err = model.ValidateTopUpQuotaCapacity(userId, creditedQuota)
	}
	if err != nil {
...
func RequestEpay(c *gin.Context) {
	...
	if req.Amount.LessThan(decimal.NewFromInt(getMinTopup())) {
	...
	callBackAddress := service.GetCallbackAddress(c.Request)
	returnUrl, _ := url.Parse(paymentReturnPathForRequest(c.Request, "/usage-logs"))
