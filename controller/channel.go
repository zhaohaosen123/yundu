...
type AddChannelRequest struct {
	...
	PoolImport                *ModelPoolImport      `json:"pool_import"`
}

type ModelPoolImport struct {
	UpstreamModels []string `json:"upstream_models"`
	ProjectModels  []string `json:"project_models"`
}
...
func AddChannel(c *gin.Context) {
	...
	if addChannelRequest.Mode == "model_pool" {
		if addChannelRequest.PoolImport == nil {
			common.ApiErrorMsg(c, "model pool selection is required")
			return
		}
		upstream := uniqueTrimmedModels(addChannelRequest.PoolImport.UpstreamModels)
		project := uniqueTrimmedModels(addChannelRequest.PoolImport.ProjectModels)
		if len(upstream) == 0 || len(project) == 0 {
			common.ApiErrorMsg(c, "upstream and project models are required")
			return
		}
		if len(upstream) > 500 || len(project) > 500 || len(upstream)*len(project) > 1000 {
			common.ApiErrorMsg(c, "too many model pool resources")
			return
		}
		channels := make([]model.Channel, 0, len(upstream)*len(project))
		for _, projectModel := range project {
			for _, upstreamModel := range upstream {
				localChannel := *addChannelRequest.Channel
				localChannel.Name = fmt.Sprintf("%s [%s -> %s]", addChannelRequest.Channel.Name, projectModel, upstreamModel)
				localChannel.Models = projectModel
				mapping, marshalErr := common.Marshal(map[string]string{projectModel: upstreamModel})
				if marshalErr != nil {
					common.ApiError(c, marshalErr)
					return
				}
				mappingValue := string(mapping)
				localChannel.ModelMapping = &mappingValue
		...
		recordManageAudit(c, "channel.create_model_pool", map[string]any{"name": addChannelRequest.Channel.Name, "count": len(channels)})
...
func uniqueTrimmedModels(models []string) []string {
