...
func TestAccessTokenTTL(t *testing.T) {
	...
	claims, err := parseAuthClaims(token, accessTokenUse, authSigningKey(accessTokenUse))
	require.NoError(t, err)
	require.NotNil(t, claims.ExpiresAt)
	assert.Equal(t, expiresAt, claims.ExpiresAt.Unix())
	assert.GreaterOrEqual(t, expiresAt, issuedAfter.Add(24*time.Hour).Unix())
	assert.LessOrEqual(t, expiresAt, issuedBefore.Add(24*time.Hour).Unix())
}
