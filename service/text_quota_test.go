...
func TestUsageForInternalStatisticsAddsTenPercentWithoutMutatingUpstreamUsage(t *testing.T) {
	cachedText := 10
	original := &dto.Usage{
		PromptTokens:     100,
		CompletionTokens: 50,
		...
		PromptTokensDetails: dto.InputTokenDetails{
			...
			CachedTokensDetails: &dto.CachedTokenDetails{
				TextTokens: &cachedText,
			},
		},
	}

	billingUsage := usageForInternalStatistics(original)
	applyUsageTokenStatisticsMultiplier(billingUsage)

	require.NotSame(t, original, billingUsage)
	assert.Equal(t, 100, original.PromptTokens)
	assert.Equal(t, 50, original.CompletionTokens)
	assert.Equal(t, 110, billingUsage.PromptTokens)
	assert.Equal(t, 55, billingUsage.CompletionTokens)
	assert.Equal(t, 165, billingUsage.TotalTokens)
	assert.Equal(t, 22, billingUsage.PromptTokensDetails.CachedTokens)
	require.NotNil(t, billingUsage.PromptTokensDetails.CachedTokensDetails)
	require.NotNil(t, billingUsage.PromptTokensDetails.CachedTokensDetails.TextTokens)
	assert.Equal(t, 11, *billingUsage.PromptTokensDetails.CachedTokensDetails.TextTokens)
}
...
func runFixedPriceAccountingCases(t *testing.T, db, logDB *gorm.DB) {
	...
	for index, tc := range []struct {
		...
		{name: "image cache stream settles usage and refunds unused reservation", expression: imageExpression, estimate: 10000, usage: imageUsage, stream: true, want: 4524, unit: billingexpr.BillingUnitToken},
		{name: "audio settlement records image cache billing inputs", expression: imageExpression, estimate: 10000, usage: imageUsage, audio: true, want: 4524, unit: billingexpr.BillingUnitToken},
		{name: "realtime records actual expression inputs", expression: imageExpression, estimate: 10000, usage: imageUsage, realtime: true, want: 4400, unit: billingexpr.BillingUnitToken},
				...
				if tc.refund {
					...
					if tc.expression == imageExpression {
						...
						if tc.realtime {
							...
							assert.Equal(t, float64(1100), billable["p"])
							assert.Equal(t, float64(0), billable["cr"])
							assert.Equal(t, float64(0), billable["img"])
						} else {
							if !tc.audio {
								assert.Equal(t, float64(330), other["cache_tokens"])
							}
							assert.Equal(t, float64(220), other["image_cache_tokens"])
							assert.Equal(t, float64(330), billable["p"])
							assert.Equal(t, float64(110), billable["cr"])
							assert.Equal(t, float64(440), billable["img"])
						}
					} else {
