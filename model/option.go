...
var deploymentOptionEnvs = map[string]string{
	"ServerAddress":         "SERVER_ADDRESS",
	"CustomCallbackAddress": "CUSTOM_CALLBACK_ADDRESS",
}
...
func loadOptionsFromDatabase() {
	...
	deploymentOverrides := getDeploymentOptionOverrides()
	for _, option := range options {
		if _, overridden := deploymentOverrides[option.Key]; overridden {
			continue
		}
	...
	for key, value := range deploymentOverrides {
		if IsPasskeyDomainOption(key) {
			passkeyOptions[key] = value
			continue
		}
		if err := updateOptionMap(key, value); err != nil {
			common.SysError("failed to apply deployment option override: " + key + ": " + err.Error())
		}
	}
	applyPasskeyDomainOptions(passkeyOptions)
	if len(deploymentOverrides) > 0 {
		common.SysLog("applied deployment option overrides: " + strings.Join(slices.Sorted(maps.Keys(deploymentOverrides)), ", "))
	}
}

func getDeploymentOptionOverrides() map[string]string {
	overrides := make(map[string]string)
	for optionKey, envName := range deploymentOptionEnvs {
		if value := strings.TrimSpace(os.Getenv(envName)); value != "" {
			overrides[optionKey] = strings.TrimRight(value, "/")
		}
	}
	return overrides
}
