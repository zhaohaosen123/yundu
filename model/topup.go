...
func (topUp *TopUp) creditedAmount() decimal.Decimal {
	if topUp.AmountDecimal != "" {
		if amount, err := decimal.NewFromString(topUp.AmountDecimal); err == nil {
			return amount
		}
	}
...
func RechargeEpay(tradeNo string, actualPaymentMethod string, callerIp string) (alreadyDone bool, err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		quotaToAdd, quotaErr = common.WalletQuotaFromDecimalStrict(
			topUp.creditedAmount().Mul(decimal.NewFromFloat(common.QuotaPerUnit)),
		)
